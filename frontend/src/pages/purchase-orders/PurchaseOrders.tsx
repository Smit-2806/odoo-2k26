...
export const PurchaseOrders: React.FC = () => {
  const { purchaseOrders, invoices, markInvoicePaid } = useProcurementStore();
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        {activePO && (
          <div className="flex items-center gap-2">
            <button
              onClick={() => alert('Invoice emailed to supplier.')}
              className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-slate-700 bg-slate-900/60 hover:bg-slate-800 text-xs font-bold text-slate-300 hover:text-white transition-all"
            >
              ...
              <div className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-7 space-y-7">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
                  {[
                    ...
                    { label: 'PO Date',      value: '21 May, 2025' },
                    { label: 'Invoice Date', value: activeInvoice?.invoiceDate || '22 May, 2025' },
                    { label: 'Due Date',     value: activeInvoice?.dueDate || '21 June, 2025' },
                  ].map(({ label, value }) => (
                    <div key={label}>
                ...
                <div className="border border-slate-800/60 rounded-xl overflow-hidden">
                  <table className="vb-table">
                    ...
                    <tbody>
                      <tr>
                        <td className="font-semibold text-slate-200">Ergonomic Chair</td>
                        <td className="text-center">25</td>
                        <td className="text-right font-mono">₹3,500</td>
                        <td className="text-right pr-4 font-semibold text-slate-200 font-mono">₹87,500</td>
                      </tr>
                      <tr>
                        <td className="font-semibold text-slate-200">Standing Desk</td>
                        <td className="text-center">10</td>
                        <td className="text-right font-mono">₹8,200</td>
                        <td className="text-right pr-4 font-semibold text-slate-200 font-mono">₹82,000</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
                ...
                <div className="flex flex-col items-end gap-2 text-xs">
                  {[
                    { label: 'Subtotal',    value: '₹1,69,500', cls: 'text-slate-400' },
                    { label: 'CGST (9%)',   value: '₹15,255',   cls: 'text-slate-400' },
                    { label: 'SGST (9%)',   value: '₹15,255',   cls: 'text-slate-400' },
                  ].map(({ label, value, cls }) => (
                  ...
                  <div className="flex justify-between w-60 border-t border-slate-800 pt-2.5 text-sm font-black">
                    ...
                    <span className="font-mono text-purple-400">₹2,00,010</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900/40 border border-slate-800/60 rounded-2xl p-6 space-y-4 no-print">
                ...
                {activeInvoice?.status === 'SUBMITTED' ? (
                  <button
                    ...
                    className="w-full py-3 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-sm font-bold text-white shadow-lg shadow-emerald-600/20 transition-all flex items-center justify-center gap-2"
                  >
